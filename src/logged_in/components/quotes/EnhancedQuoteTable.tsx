...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler = (property: keyof Quote) => (
    event: React.MouseEvent<unknown>
  ) => {
...
export default function EnhancedQuoteTable(props: Props) {
  const classes = useStyles();
  const [order, setOrder] = React.useState<Order>("asc");
  const [orderBy, setOrderBy] = useState<keyof Quote>("customerName");
  const [selected, setSelected] = React.useState<string[]>([]);
  const [page, setPage] = React.useState(0);
  const [dense, setDense] = React.useState(false);
  const [rowsPerPage, setRowsPerPage] = React.useState(5);

  const { onViewQuote, data: rows } = props;
                ...
                .map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="left">{row.quoteNumber}</TableCell>
                      <TableCell align="left">{row.issueDate}</TableCell>

                      <TableCell align="left">{row.dueDate}</TableCell>

                      <TableCell align="right">
                        <QuoteMenuComp onViewQuote={onViewQuote} row={row} />
                      </TableCell>
                    </TableRow>
                  );
                })}
